...
${l.slice(0,4e3)}`),console.error(e.error);return}const sourceMediaVersion=e.value.imageVersion==="v1"?"v1":"v2",r=e.value.asWritable().purify().modify(l=>{l.visibility=N.PRIVATE,l.imageVersion=sourceMediaVersion;});typeof e.value.simpleDescription==="string"&&e.value.simpleDescription.length>0&&(r.simpleDescription=e.value.simpleDescription);const l=movingCoverCandidates(e.value),f=r.isMovingPortraitImage||l.some(k=>/(gif|anim|moving|motion)/i.test(k.key)||/\.gif(?:[?#]|$)/i.test(k.url));if(f){const y=e.value.portraitImage?.image("origin")??e.value.profileImage?.image("origin");if(!y){g().doToastifyAlert("움직이는 원본 표지 주소를 찾지 못했어요.",1e4);return}r.portraitImageUrl=y,r.isMovingPortraitImage=!0;}const s=r.imageVersion==="v2"&&r.sets.some(l=>l.situationImages.length>0&&l.imageMatrix!==void 0),o=await v().pullNewId();if(!o.ok){const l=o.error instanceof Error?o.error.message:String(o.error);g().doToastifyAlert(`복사본 준비 실패
...
${o.slice(0,4e3)}`,1e4),console.error(s.error);return}try{const o=s.value.asWritable().purify(),movingCandidates=movingCoverCandidates(s.value),hadMovingCover=o.isMovingPortraitImage||movingCandidates.some(d=>/(gif|anim|moving|motion)/i.test(d.key)||/\.gif(?:[?#]|$)/i.test(d.url));e?o.dematrix():o.normalizeMediaV2();if(hadMovingCover){const d=s.value.portraitImage?.image("origin")??s.value.profileImage?.image("origin");if(!d){g().doToastifyAlert("미디어 형식을 바꾸지 못했어요.\n움직이는 원본 표지 주소를 찾지 못했어요.",1e4);return}o.portraitImageUrl=d,o.isMovingPortraitImage=!0}const hasSituationImages=o.sets.some(d=>d.situationImages.length>0),a=await v().edit(n.id,o,hasSituationImages);if(!a.ok){const c=a.error instanceof Error?a.error.message:String(a.error);g().doToastifyAlert(`미디어 형식 변경에 실패했어요.

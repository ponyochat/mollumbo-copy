...
${l.slice(0,4e3)}`),console.error(e.error);return}const sourceMediaVersion=e.value.imageVersion==="v1"?"v1":"v2",r=e.value.asWritable().purify().modify(l=>{l.visibility=N.PRIVATE,l.imageVersion=sourceMediaVersion;});typeof e.value.simpleDescription==="string"&&e.value.simpleDescription.length>0&&(r.simpleDescription=e.value.simpleDescription);const l=movingCoverCandidates(e.value),f=r.isMovingPortraitImage||l.some(k=>/(gif|anim|moving|motion)/i.test(k.key)||/\.gif(?:[?#]|$)/i.test(k.url));if(f){if(l.length===0){g().doToastifyAlert("움직이는 원본 표지를 찾지 못했어요.");return}g().doToastifyAlert("움직이는 표지를 새 복사본에 올리는 중이에요.",6e5);let y=null,k=null;for(const S of l)try{const x=await bt(S.url);if(!await isAnimatedCoverBlob(x.blob,S)){k=new Error("움직이는 표지 파일이 아닌 후보는 건너뛰었어요.");continue}if(x.blob.size<=5*1024*1024){y=x.blob;break}k=new Error("표지 파일이 5MB를 넘어요.");}catch(x){k=x,console.warn("움직이는 표지 후보 다운로드 실패",S.url,x);}if(!y){const S=k instanceof Error?k.message:String(k??"원본 파일을 찾지 못했어요.");g().doToastifyAlert(`움직이는 표지를 복사할 수 없어요.
${S.slice(0,4e3)}`,1e4);return}const S=await v().uploadStoryPortraitImage(await Xe(y));if(!S.ok){const x=S.error instanceof Error?S.error.message:String(S.error);g().doToastifyAlert(`움직이는 표지 업로드 실패
${x.slice(0,4e3)}`),console.error(S.error);return}r.portraitImageUrl=S.value,r.isMovingPortraitImage=!0;}const s=r.imageVersion==="v2"&&r.sets.some(l=>l.situationImages.length>0&&l.imageMatrix!==void 0),o=await v().pullNewId();if(!o.ok){const l=o.error instanceof Error?o.error.message:String(o.error);g().doToastifyAlert(`복사본 준비 실패
...
${o.slice(0,4e3)}`,1e4),console.error(s.error);return}try{const o=s.value.asWritable().purify(),movingCandidates=movingCoverCandidates(s.value),hadMovingCover=o.isMovingPortraitImage||movingCandidates.some(d=>/(gif|anim|moving|motion)/i.test(d.key)||/\.gif(?:[?#]|$)/i.test(d.url));e?o.dematrix():o.normalizeMediaV2();if(hadMovingCover){g().doToastifyAlert("움직이는 표지를 그대로 유지하는 중이에요.",6e5);let d=null,u=null;for(const p of movingCandidates)try{const w=await bt(p.url);if(!await isAnimatedCoverBlob(w.blob,p)){u=new Error("움직이는 표지 파일이 아닌 후보는 건너뛰었어요.");continue}if(w.blob.size<=5*1024*1024){d=w.blob;break}u=new Error("표지 파일이 5MB를 넘어요.")}catch(w){u=w,console.warn("미디어 변경 중 움직이는 표지 다운로드 실패",p.url,w)}if(!d){const p=u instanceof Error?u.message:String(u??"원본 파일을 찾지 못했어요.");g().doToastifyAlert(`미디어 형식을 바꾸지 못했어요.\n움직이는 표지를 유지할 수 없어요.\n${p.slice(0,4e3)}`,1e4);return}const p=await v().uploadStoryPortraitImage(await Xe(d));if(!p.ok){const w=p.error instanceof Error?p.error.message:String(p.error);g().doToastifyAlert(`미디어 형식을 바꾸지 못했어요.\n움직이는 표지 업로드 실패\n${w.slice(0,4e3)}`,1e4);return}o.portraitImageUrl=p.value,o.isMovingPortraitImage=!0}const a=await v().edit(n.id,o,true);if(!a.ok){const c=a.error instanceof Error?a.error.message:String(a.error);g().doToastifyAlert(`미디어 형식 변경에 실패했어요.
